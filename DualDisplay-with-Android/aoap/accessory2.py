...
def accessory_task(vid):
	...
	if dev.idProduct in ACCESSORY_PID:
		...
		print("device is not in accessory mode yet, VID %x0.4x" % vid)
...
#	writer_thread.start()
	print("read from android...")
	length = -1
	while True:
		try:
			data = ep_in.read(size = 1, timeout = 1)
			print("read value %d" % data[0])
		except usb.core.USBError as e:
			print("failed to send IN trasfer")
			print(e)
			break
